...
public class AutoReplyService extends AccessibilityService {
    ...
    public void onAccessibilityEvent(final AccessibilityEvent event) {
        ...
        Log.e("maptrix", "get event = " + eventType);
        switch (eventType) {
            case AccessibilityEvent.TYPE_NOTIFICATION_STATE_CHANGED:// 通知栏事件
                Log.e("maptrix", "get notification event");
                    ...
                    for (CharSequence text : texts) {
                        ...
                        if (!TextUtils.isEmpty(content)) {
                            if (isScreenLocked()) {
                                locked = true;
                                wakeAndUnlock();
                                Log.e("maptrix", "the screen is locked");
                                if (isAppForeground(MM_PNAME)) {
                                    background = false;
                                    Log.e("maptrix", "is mm in foreground");
                                    ...
                                    background = true;
                                    Log.e("maptrix", "is mm in background");
                                    sendNotifacationReply(event);
                                }
                            } else {
                                locked = false;
                                Log.e("maptrix", "the screen is unlocked");
                                if (isAppForeground(MM_PNAME)) {
                                    background = false;
                                    Log.e("maptrix", "is mm in foreground");
                                    sendNotifacationReply(event);
            ...
            case AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED:
                Log.e("maptrix", "get type window down event");
                if (!hasAction) break;
                itemNodeinfo = null;
    ...
    private boolean fill() {
        ...
        if (rootNode != null) {
            String replyWords ="我是自动回复";
            AutoManager.ReplyKeywordsChangeListenner listener = AutoManager.getInstance().getReplyListener();
            if (listener != null){
               replyWords =  listener.getReplyWords();
            }
            return findEditText(rootNode,replyWords);
        }
        return false;
    }
    ...
    private boolean findEditText(AccessibilityNodeInfo rootNode, String content) {
        ...
        Log.e("maptrix", "root class=" + rootNode.getClassName() + ","+ rootNode.getText()+","+count);
        for (int i = 0; i < count; i++) {
            AccessibilityNodeInfo nodeInfo = rootNode.getChild(i);
            if (nodeInfo == null) {
                Log.e("maptrix", "nodeinfo = null");
                continue;
            }

            Log.e("maptrix", "class=" + nodeInfo.getClassName());
    ...
    private void release() {
        ...
        if (locked && kl != null) {
            Log.e("maptrix", "release the lock");
            //得到键盘锁管理器对象
            kl.reenableKeyguard();
            locked = false;
        }
